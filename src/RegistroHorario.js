...
function RegistroHorario() {
  ...
  const [alerta, setAlerta] = useState("no");
  ...
  const toggleAlert = () => {
    if (alerta === "no") {
      setAlerta("si");
      alert("La alerta se encuentra activada");
    } else {
      setAlerta("no");
      alert("La alerta se encuentra desactivada");
    }
  };
  const resetAlert = () => setAlerta("no");
      ...
      <Table striped bordered hover id="tabla-registro">
        <thead>
          <tr>
            ...
            {user === "Richard Barrera" && <th>Número de Teléfono</th>}
            <th>Empresa</th>
            <th>Fecha</th>
